...
(function () {
  try {
    ...
    if (logArr.length > 0) {
      ritmGR.work_notes =
        current.getDisplayValue() + " execution log:\n" + logArr.join("\n");
    }
    ...
    return (
      "Exception in x_tieoy_eus_device_transform [" +
...
function createTask(ritmGR) {
  ...
  var description =
    "User needs a computer - assign one from the on-site stock!\n\n";
  //description += '\n' + ritmGR.description;
  description += new global.EusTicketIntegration().getRitmVariablesPrintout(
    ritmGR
  );
  ...
  taskGR.location = "6ba1e5fe93fd76d0c26073258aba10a4"; // Finland fallback
  if (!JSUtil.nil(callerLocation)) {
    taskGR.location = callerLocation;
  }
  taskGR.service_offering = "100ac4d71b3778d4c5f8dc2c9b4bcb86"; // FSO On-site by Experis/Manpower
